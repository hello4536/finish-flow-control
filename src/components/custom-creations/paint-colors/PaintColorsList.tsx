...
const PaintColorsList: React.FC<PaintColorsListProps> = ({
  paintColors,
  isLoading,
  deletePaintColor
}) => {
  ...
  return <div>
      <h2 className="text-xl font-bold mb-4 text-blue-600">Saved Colors ({paintColors?.length || 0})</h2>
      
      {paintColors && paintColors.length > 0 ? <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {paintColors.map(color => <Card key={color.id} className="group overflow-hidden">
              <CardContent className="p-0">
                <div className="p-4">
                  <div className="w-full h-20 rounded-md mb-3" style={{
                    ...
                    <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <Dialog>
                        <DialogTrigger asChild>
                          <Button variant="ghost" size="icon" onClick={() => setSelectedColor(color)}>
                        ...
                        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
                          ...
                          {selectedColor && <div className="mt-4">
                              <Tabs defaultValue="details">
                                ...
                                <TabsContent value="details" className="space-y-4">
                                  <div className="flex items-center gap-4">
                                    <div className="w-16 h-16 rounded-md" style={{
                      ...
                      <Button variant="ghost" size="icon" onClick={() => deletePaintColor.mutate(color.id)} disabled={deletePaintColor.isPending}>
                        <Trash2 className="h-4 w-4 text-destructive" />
                      </Button>
                    </div>
                  </div>
                  
                  {color.substrate_type && <Badge className="mt-2" variant="outline">{color.substrate_type}</Badge>}
        ...
        </div> : <div className="text-center py-10 text-muted-foreground border rounded-lg">
