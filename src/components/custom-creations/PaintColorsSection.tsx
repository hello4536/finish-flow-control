...
const PaintColorsSection: React.FC<PaintColorsSectionProps> = ({
  ...
  return <div className="space-y-6">
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            field
                  ...
                  <FormControl>
                    <Input placeholder="Ocean Blue" className="rounded-sm" />
                  </FormControl>
                  <FormMessage />
                </FormItem>} />
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:col-span-2">
              <FormField control={form.control} name="hexCode" render={({
              field
            }) => <FormItem>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div className="h-10 w-10 rounded-md border border-input" style={{
              ...
              <FormField control={form.control} name="rgbValues" render={({
              field
            }) => <FormItem>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <FormField control={form.control} name="cmykValues" render={({
            field
          }) => <FormItem>
            ...
            <FormField control={form.control} name="labValues" render={({
            field
          }) => <FormItem>
            ...
            <FormField control={form.control} name="deltaE" render={({
            field
          }) => <FormItem>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <FormItem>
              ...
              <div className="flex items-center gap-2">
                ...
                {swatchImage && <div className="h-10 w-10 rounded border border-input overflow-hidden">
                    <img src={URL.createObjectURL(swatchImage)} alt="Color swatch preview" className="h-full w-full object-cover" />
                  </div>}
              </div>
            </FormItem>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <FormField control={form.control} name="substrateType" render={({
              field
            }) => <FormItem>
                    ...
                    <Select onValueChange={field.onChange} defaultValue={field.value}>
                      ...
                      <SelectContent>
                        {substrates.map(substrate => <SelectItem key={substrate} value={substrate}>{substrate}</SelectItem>)}
                      </SelectContent>
                    </Select>
                    <FormMessage />
                  </FormItem>} />
              
              <FormField control={form.control} name="applicationMethod" render={({
              field
            }) => <FormItem>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <FormField control={form.control} name="humidity" render={({
              field
            }) => <FormItem>
              ...
              <FormField control={form.control} name="boothLighting" render={({
              field
            }) => <FormItem>
            ...
            <FormField control={form.control} name="environmentalNotes" render={({
            field
          }) => <FormItem>
                  <FormLabel>Environmental Notes</FormLabel>
                  <FormControl>
                    <Textarea placeholder="Notes about environmental conditions during color creation" className="min-h-[80px]" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>} />
            ...
            field
                  ...
                  <FormControl>
                    <Textarea placeholder="Any other notes about this color" className="min-h-[80px]" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>} />
          </div>
          
          <Button type="submit" disabled={addPaintColor.isPending || uploadingImage}>
      ...
      {isLoading ? <div className="flex justify-center py-10">
        ...
        </div> : paintColors && paintColors.length > 0 ? <>
          <div className="text-sm text-muted-foreground mb-2">
            {paintColors.length} color{paintColors.length !== 1 ? 's' : ''} saved
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {paintColors.map(color => <Card key={color.id} className="group overflow-hidden">
                <CardContent className="p-4 flex flex-col gap-2">
                  <div className="w-full h-24 rounded-md" style={{
              backgroundColor: color.hex_code
                  ...
                  <div className="flex justify-between items-center">
                    <div>
                      ...
                      {color.rgb_values && <p className="text-xs text-muted-foreground">RGB: {color.rgb_values}</p>}
                    </div>
                    <Button variant="ghost" size="icon" className="opacity-0 group-hover:opacity-100" onClick={() => deletePaintColor.mutate(color.id)} disabled={deletePaintColor.isPending}>
                      <Trash2 className="h-4 w-4 text-destructive" />
                    </Button>
                  </div>
                  
                  {(color.substrate_type || color.application_method) && <div className="mt-2 text-xs text-muted-foreground">
                  ...
                  {color.swatch_image && <div className="mt-2">
                      <img src={color.swatch_image} alt={`${color.name} swatch`} className="w-full h-20 object-cover rounded-md" />
                    </div>}
                  
                  {color.notes && <p className="text-sm text-muted-foreground mt-2 line-clamp-2">
                      {color.notes}
                    </p>}
                  
                  {color.created_by && <p className="text-xs text-muted-foreground mt-1">
