...
interface StainsListProps {
  ...
  updateStain: UseMutationResult<any, Error, Partial<Stain> & {
    id: string;
  }, unknown>;
}
const StainsList: React.FC<StainsListProps> = ({
  stains,
  isLoading,
  deleteStain,
  updateStain
  ...
  return <div>
      <h2 className="text-xl font-bold mb-4 text-blue-600">Saved Stains ({stains.length})</h2>
      
      {stains.length > 0 ? <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {stains.map(stain => <StainListItem key={stain.id} stain={stain} deleteStain={deleteStain} onEdit={handleEditStain} onViewDetails={handleViewDetails} />)}
        </div> : <StainsEmptyState />}
      
      <StainDetailDialog stain={selectedStain} open={detailsDialogOpen} onOpenChange={setDetailsDialogOpen} />
    </div>;
};
