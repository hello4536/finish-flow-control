  ...
  return <Card className="lg:col-span-4">
      ...
      <CardContent>
        {isLoading ? <div className="flex justify-center items-center h-[300px]">
          ...
          </div> : tasks.length > 0 ? <div className="space-y-4">
            {tasks.map(task => <div key={task.id} className="flex items-center justify-between border-b pb-4">
                ...
                <div className="flex items-center">
                  {task.status === "pending" ? <Button size="sm" variant="outline" onClick={() => completeTask(task.id)}>
                      Complete
                    </Button> : <span className="text-xs text-green-600 font-medium flex items-center">
                      <CheckSquare className="h-3.5 w-3.5 mr-1" /> Completed
                    </span>}
                </div>
              </div>)}
          </div> : <div className="flex justify-center items-center h-[300px] flex-col">
