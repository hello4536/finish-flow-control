...
const DocumentsSection: React.FC<DocumentsSectionProps> = ({
  ...
  return <div className="space-y-6">
      <div className="flex flex-col items-center justify-center border-2 border-dashed rounded-md p-10 border-border">
        ...
        <Input type="file" ref={fileInputRef} onChange={handleFileChange} className="hidden" multiple accept=".pdf,.doc,.docx,.txt,.jpg,.jpeg,.png,.csv,.xlsx" id="file-upload" />
        <Button onClick={() => fileInputRef.current?.click()} disabled={uploadDocument.isPending} className="bg-blue-600 hover:bg-blue-500">
      ...
      {documents.length > 0 ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        return <Card key={doc.id} className="overflow-hidden group">
                <div className="cursor-pointer" onClick={() => previewDocument(doc)}>
                  {isImage ? <AspectRatio ratio={16 / 9}>
                      <img src={doc.url} alt={doc.name} className="w-full h-full object-cover" />
                    </AspectRatio> : <div className="bg-muted h-32 flex items-center justify-center">
                ...
                <CardContent className="p-4">
                  <div className="flex items-start justify-between">
                    ...
                    <Button variant="ghost" size="icon" className="opacity-0 group-hover:opacity-100" onClick={e => {
                e.stopPropagation();
