  ...
  return <div className="space-y-6">
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
          ...
          <Button type="submit" disabled={addNote.isPending} className="w-full md:w-auto bg-blue-600 hover:bg-blue-500">
            {addNote.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <FileText className="mr-2 h-4 w-4" />}
            Save Note
          </Button>
        </form>
      </Form>
      ...
      {notes.length > 0 ? <div className="space-y-4">
          {notes.map(note => <Card key={note.id} className="group">
              <CardContent className="p-4">
                <div className="flex items-center justify-between mb-2">
                  <h3 className="font-medium">{note.title}</h3>
                  <Button variant="ghost" size="icon" className="opacity-0 group-hover:opacity-100" onClick={() => handleDeleteNote(note.id)} disabled={deleteNote.isPending}>
