...
const Header: React.FC = () => {
  ...
  return <header className="sticky top-0 z-50 bg-white border-b border-gray-100 shadow-sm">
      <div className="container mx-auto px-4 py-4 flex items-center justify-between">
        ...
        {/* Mobile menu button */}
        <button className="md:hidden p-2 text-gray-600 hover:text-blue-500" onClick={toggleMobileMenu}>
        ...
        <div className="hidden md:flex items-center space-x-4">
          {user ? <>
              ...
              <Button size="sm" variant="ghost" onClick={handleSignOut} className="rounded-md hover:bg-blue-50">
                Sign Out
              </Button>
            </> : <>
      ...
      {mobileMenuOpen && <div className="md:hidden bg-white border-t border-gray-100 py-4">
          <div className="container mx-auto px-4 flex flex-col space-y-4">
            <Link to="/" className="px-4 py-2 text-gray-600 hover:bg-gray-50 rounded-md" onClick={() => setMobileMenuOpen(false)}>
              Home
            </Link>
            <Link to="/woodworking-finishing" className="px-4 py-2 text-gray-600 hover:bg-gray-50 rounded-md" onClick={() => setMobileMenuOpen(false)}>
              Woodworking
            </Link>
            <Link to="/auto-body-finishing" className="px-4 py-2 text-gray-600 hover:bg-gray-50 rounded-md" onClick={() => setMobileMenuOpen(false)}>
              Auto Body
            </Link>
            <Link to="/#pricing" className="px-4 py-2 text-gray-600 hover:bg-gray-50 rounded-md" onClick={() => setMobileMenuOpen(false)}>
              Pricing
            </Link>
            
            <div className="pt-2 border-t border-gray-100 flex flex-col space-y-2">
              {user ? <>
                  <Button asChild className="w-full justify-center rounded-md bg-blue-900 hover:bg-blue-800" onClick={() => setMobileMenuOpen(false)}>
                    <Link to="/dashboard">Dashboard</Link>
                  </Button>
                  <Button variant="outline" className="w-full justify-center rounded-md" onClick={() => {
              handleSignOut();
              setMobileMenuOpen(false);
                  ...
                  <Button asChild variant="outline" className="w-full justify-center rounded-md" onClick={() => setMobileMenuOpen(false)}>
