      ...
      <Card className="overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 border-2 border-primary/10">
        ...
        <div className="px-4 pb-8">
          <div className="overflow-hidden rounded-lg relative group">
            <img alt={imageAlt} className="w-full h-auto rounded-lg shadow-lg group-hover:scale-110 transition-transform duration-500 cursor-pointer" src={image} onClick={() => setIsOpen(true)} />
...
const FeaturesSection = () => {
  ...
  return <section id="features" className="py-[70px] bg-slate-50">
      <div className="container px-6 mx-auto">
        ...
        <div className="grid gap-12 md:grid-cols-2 max-w-[1400px] mx-auto">
          {features.map((feature, index) => <FeatureCard key={index} icon={feature.icon} title={feature.title} description={feature.description} image={feature.image} imageAlt={feature.imageAlt} iconBackground={feature.iconBackground} titleColor={feature.titleColor} descriptionColor={feature.descriptionColor} />)}
        </div>
      </div>
    </section>;
};
