...
const InspectionsTab: React.FC<InspectionsTabProps> = ({
  ...
  return <>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="bg-sky-100">Product</TableHead>
                <TableHead className="bg-sky-100">Inspector</TableHead>
                <TableHead className="bg-sky-100">Status</TableHead>
                <TableHead className="bg-sky-100">Actions</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {inspections.length > 0 ? inspections.map(inspection => <TableRow key={inspection.id}>
                    ...
                    <TableCell>
                      <div className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium
                        ${inspection.status === "Passed" ? "bg-green-100 text-green-800" : inspection.status === "Failed" ? "bg-red-100 text-red-800" : "bg-yellow-100 text-yellow-800"}`}>
                        {inspection.status}
                      </div>
                    </TableCell>
                    <TableCell>
                      <div className="flex gap-2">
                        <Button variant="ghost" size="sm" onClick={() => handleEdit(inspection)} className="h-8 w-8 p-0">
                          <Edit className="h-4 w-4" />
                          <span className="sr-only">Edit</span>
                        </Button>
                        <Button variant="ghost" size="sm" onClick={() => handleDelete(inspection)} className="h-8 w-8 p-0 text-destructive hover:text-destructive">
      ...
      <EditInspectionDialog inspection={selectedInspection} open={editDialogOpen} onOpenChange={setEditDialogOpen} onSaveInspection={handleSaveInspection} />
      
      <DeleteInspectionDialog inspectionId={inspectionToDelete} open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen} onConfirmDelete={handleConfirmDelete} />
    </>;
};
