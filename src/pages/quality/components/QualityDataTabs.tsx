...
const QualityDataTabs: React.FC<QualityDataTabsProps> = ({
  ...
  const filteredInspections = inspections.filter(item => item.inspection_id.toLowerCase().includes(search.toLowerCase()) || item.product.toLowerCase().includes(search.toLowerCase()) || item.inspector.toLowerCase().includes(search.toLowerCase()));
  return <Card>
      <CardContent className="p-0">
        <Tabs defaultValue="inspections" className="w-full">
          ...
          <TabsContent value="inspections" className="p-4">
            <InspectionsTab inspections={filteredInspections} isLoading={isLoading} />
          </TabsContent>
        </Tabs>
      </CardContent>
    </Card>;
